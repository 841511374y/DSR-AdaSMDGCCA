...
def norm(X):
    X0 = X - torch.mean(X)
    Y = X0 / (torch.sqrt(sum(pow(X0, 2))))
    return Y


def get_connectivity(data, alpha, device):
    ...
    Penalty = nn.Parameter(torch.tensor(Penalty)).to(device)
    return Penalty


def frob(z):
    vec = torch.reshape(z, [-1])
    return torch.sum(torch.sum(torch.multiply(vec, vec)))
...
class SDGCCA_3_M(nn.Module):
    ...
    def cal_loss(self, H_list, device, train=True):
        eps = 1e-8
        AT_list = []
        ...
        Xu = torch.matmul(H_list[0], self.U[0])
        Yv = torch.matmul(H_list[1], self.U[1])
        Zw = torch.matmul(H_list[2], self.U[2])
        omega12 = 1 / norm(Xu - Yv)
        omega23 = 1 / norm(Yv - Zw)
        omega13 = 1 / norm(Xu - Zw)
        a1 = omega12 * frob(G - Xu)
        a2 = omega23 * frob(G - Yv)
        a3 = omega13 * frob(G - Zw)
        L1 = get_connectivity(H_list[0], 1, device)
        L2 = get_connectivity(H_list[1], 1, device)
        L3 = get_connectivity(H_list[2], 1, device)
        b1 = frob(self.U[0].transpose(0, 1) * L1 * self.U[0])
        b2 = frob(self.U[1].transpose(0, 1) * L2 * self.U[1])
        b3 = frob(self.U[2].transpose(0, 1) * L3 * self.U[2])
    ...
    def predict(self, x1, x2, x3):
        ...
        y_hat1 = F.sigmoid(y_hat1)
        y_hat2 = F.sigmoid(y_hat2)
        y_hat3 = F.sigmoid(y_hat3)
        y_ensemble = F.sigmoid(y_ensemble)
